...
int main()
{
  ...
  h.onMessage([&pid](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
	  ...
	  std::cout << "Strating Update Error" << std::endl;
	  ...
	  else 
	  {
	  	if (pid.getTimeSteps() > 25000)
	   		msgJson["throttle"] = (1 - std::abs(steer_value)) * 0.25 + 0.2;
	  	else if (pid.getTimeSteps() > 20000)
           		msgJson["throttle"] = 0.1;
	  	else if (pid.getTimeSteps() > 20000)
           		msgJson["throttle"] = 0.08;
	  	else if (pid.getTimeSteps() > 10000)
           		msgJson["throttle"] = 0.05;
	  	else if (pid.getTimeSteps() >= 5000)
           		msgJson["throttle"] = 0.05;
	  	else if (pid.getTimeSteps() < 5000)
           		msgJson["throttle"] = 0.05;
	  }
